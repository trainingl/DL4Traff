...
class TemporalBlock(nn.Module):
    def __init__(self, hidden_dim, dropout, num_layers):
        ...
        self.temporal_layers = nn.Sequential(
            *[TemporalMLP(input_dim=hidden_dim, output_dim=hidden_dim, dropout=dropout) for _ in range(num_layers)]
        )
        
    def forward(self, input_data):
        # input_data shape: (B, D, N, 1)
        hidden = self.temporal_layers(input_data)
        return hidden


class E2MLP(nn.Module):
    def __init__(self, num_nodes, input_dim, input_len, output_len, dropout, num_layers, num_blocks,
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        for i in range(0, self.num_blocks):
            ...
            # 4.2 temporal mlp blocks
            hidden = self.temporal_blocks[i](hidden)
            # 4.3 forcast layer
