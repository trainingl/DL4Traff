...
class MultiLayerPerceptron(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        """
        hidden = self.fc2(self.dropout(self.act(self.fc1(input_data))))
...
class STEMLP(nn.Module):
    def __init__(self, num_nodes, input_dim, embed_dim, input_len, output_len, node_dim, lape_dim, 
                 adp_dim, adj_mx, num_layerA, num_layerB, num_layerC, time_of_day_size, day_of_week_size, 
        ...
        self.lap_pe = self._calculate_laplacian_pe(adj_mx)
        ...
        if self.if_time_of_day or self.if_day_of_week:
            self.temporal_hidden_dim = self.embed_dim + self.node_dim * int(self.if_time_of_day) + self.node_dim * int(self.if_day_of_week)
            self.temporal_encoder = nn.Sequential(
                *[MultiLayerPerceptron(self.temporal_hidden_dim, int(self.temporal_hidden_dim * 1.5)) for _ in range(self.num_layerA)]
            )
            self.hidden_dim += self.temporal_hidden_dim
        if self.if_adp_spatial or self.if_pre_spatial:
            self.spatial_hidden_dim = self.embed_dim + self.lape_dim * int(self.if_pre_spatial) + self.lape_dim * int(self.if_adp_spatial)
            self.spatial_encoder = nn.Sequential(
                *[MultiLayerPerceptron(self.spatial_hidden_dim, int(self.spatial_hidden_dim * 1.5)) for _ in range(self.num_layerB)]
            )
            self.hidden_dim += self.spatial_hidden_dim
    ...
    def _calculate_laplacian_pe(self, adj_mx):
        """Transform the graph structure (road network) into a vector representation in Euclidean space."""
        L, isolated_point_num = self._calculate_normalized_laplacian(adj_mx)
        EigVal, EigVec = np.linalg.eig(L.toarray())  # Eigenvalue decomposition
        idx = EigVal.argsort()
    ...
    def _calculate_sym_normalized_laplacian(self, adj):
        ...
        EigVal, EigVec = torch.linalg.eigh(adj)  # EigVal.shape: (N, ), EigVec.shape: (N, N)
        idx = torch.argsort(torch.abs(EigVal), dim=0, descending=True)
        EigVal, EigVec = EigVal[idx], EigVec[:, idx] # sort in descending order
        laplacian_pe = EigVec[:, isolated_point_num + 1: self.lape_dim + isolated_point_num + 1]  # K个最小的非平凡特征向量
        return laplacian_pe
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        if self.if_adp_spatial:
            ...
            adp_lap_pe = self._calculate_sym_normalized_laplacian(adp_mx)
            # (N, d) -> (1, d, N, 1)
            adp_lap_pe = self.adp_spatial_embedding(adp_lap_pe).expand(batch_size, -1, -1, -1)
            node_emb.append(adp_lap_pe)
        if self.if_pre_spatial:
            pre_lap_pe = self.lap_pe.to(input_data.device)
            # (N, d) -> (1, d, N, 1)
            pre_spatial_emb = self.pre_spatial_embedding(pre_lap_pe).expand(batch_size, -1, -1, -1)
        ...
        # 4.spatial and temporal dual branch encoder
        if self.if_adp_spatial or self.if_pre_spatial:
            s_emb = torch.cat([time_series_emb] + node_emb, dim=1)
            ds_emb = self.spatial_encoder(s_emb)
        if self.if_time_of_day or self.if_day_of_week:
            t_emb = torch.cat([time_series_emb] + temp_emb, dim=1)
            dt_emb = self.temporal_encoder(t_emb)
...
if __name__ == '__main__':
    adj_mx = load_pickle("./datasets/PMBS08/adj_PEMS08.pkl")
    model = STEMLP(
        num_nodes=170, 
        input_dim=3, 
        embed_dim=96, 
